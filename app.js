...
app.post('/f/:slug/submit',auth,async(req,res)=>{const f=await getSlug(req.params.slug);if(!f)return res.status(404).send('Form not found');const answers={};for(const q of arr(f.questions))answers[q.title]=String(req.body['answer_'+q.title]||'').slice(0,5000);const s={id:crypto.randomUUID(),form_id:f.id,user_id:req.session.user.id,username:req.session.user.global_name||req.session.user.username||'User',answers,status:'pending',created_at:new Date().toISOString()};await createSubmission(s);res.send(page(req,'Submitted','<main class="wrap"><div class="card empty"><h2>Application submitted</h2><p class="muted">Thanks. Your submission is now pending review.</p><a class="btn" href="/dashboard">Back to dashboard</a></div></main>'));});
