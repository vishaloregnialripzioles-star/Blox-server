...
app.get('/server/:guildId/forms/new', auth, (req, res) => {
  ...
  const typeOptions=[['staff','Staff Application'],['appeal','Appeal'],['partnership','Partnership'],['report','Report'],['custom','Custom Form']].map(([v,l])=>'<option value="'+v+'">'+l+'</option>').join('');
  const defsJson=JSON.stringify(defaults).replace(/</g,'\\u003c');
  const body=`<main class="wrap"><div class="hero"><h1>Create a form</h1><p class="muted">Add as many questions as you need. There is no 5-question limit.</p></div><form method="post" action="/server/${encodeURIComponent(g.id)}/forms/create"><div class="card"><div class="grid"><div class="field"><label>Form name</label><input class="input" name="name" required placeholder="Staff Application"></div><div class="field"><label>Type</label><select class="select" name="form_type" id="formType">${typeOptions}</select></div></div><div class="field"><label>Description</label><textarea class="textarea" name="description"></textarea></div><div class="row"><div><h3 style="margin:0">Questions</h3><div class="muted small">Add, remove and edit any number of questions.</div></div><span class="pill" id="questionCount"></span></div><div id="questions"></div><div class="actions" style="margin-top:14px"><button type="button" class="btn alt" id="addQuestion">＋ Add Question</button><button type="button" class="btn alt" id="loadDefaults">↻ Load Type Defaults</button></div><div class="grid" style="margin-top:20px"><div class="field"><label>Reviewer User IDs</label><input class="input" name="reviewer_users"></div><div class="field"><label>Reviewer Role IDs</label><input class="input" name="reviewer_roles"></div></div><div class="actions"><button class="btn" type="submit">Save Form</button><a class="btn alt" href="/server/${encodeURIComponent(g.id)}">Cancel</a></div></div></form></main><script>const defs=${defsJson};let qs=[];const escA=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));function syncQuestions(){const box=document.getElementById('questions');if(!box)return;qs=Array.from(box.querySelectorAll('.question')).map(el=>({title:el.querySelector('[name="q_title"]')?.value||'',text:el.querySelector('[name="q_text"]')?.value||'',type:el.querySelector('[name="q_type"]')?.value||'short'}))}function addQ(q={title:'',text:'',type:'short'}){syncQuestions();qs.push({...q});render()}function loadDefaults(){qs=(defs[document.getElementById('formType').value]||[]).map(q=>({...q}));render()}function render(){const box=document.getElementById('questions');document.getElementById('questionCount').textContent=qs.length+(qs.length===1?' question':' questions');box.innerHTML=qs.map((q,i)=>\`<div class="question"><div class="row"><b>Question \${i+1}</b><button type="button" class="btn danger remove-q">Remove</button></div><div class="qrow"><div class="field"><label>Title</label><input class="input" name="q_title" value="\${escA(q.title)}" required></div><div class="field"><label>Question</label><input class="input" name="q_text" value="\${escA(q.text)}" required></div><div class="field"><label>Answer type</label><select class="select" name="q_type"><option value="short" \${q.type==='short'?'selected':''}>Short text</option><option value="long" \${q.type==='long'?'selected':''}>Long text</option><option value="number" \${q.type==='number'?'selected':''}>Number</option></select></div></div></div>\`).join('');box.querySelectorAll('.remove-q').forEach((b,i)=>b.onclick=()=>{syncQuestions();qs.splice(i,1);render()})}document.getElementById('addQuestion').onclick=()=>addQ();document.getElementById('loadDefaults').onclick=loadDefaults;loadDefaults();</script>`; res.send(page(req,'Create Form',body));
});

app.post('/server/:guildId/forms/create',auth,async(req,res)=>{const g=manageable(req).find(x=>x.id===req.params.guildId);if(!g)return res.status(403).send('Forbidden');let slug=slugify(req.body.name);const duplicate=async()=>pool?(await pool.query('select id from sparxie_forms where slug=$1',[slug])).rows.length:memory.forms.some(x=>x.slug===slug);if(await duplicate())slug+='-'+crypto.randomBytes(2).toString('hex');const formType=['staff','appeal','partnership','report','custom'].includes(String(req.body.form_type))?String(req.body.form_type):'custom';const f={id:crypto.randomUUID(),guild_id:g.id,name:String(req.body.name||'Untitled Form').trim().slice(0,100),slug,form_type:formType,description:String(req.body.description||'').slice(0,1000),questions:parseQuestions(req.body),reviewer_users:parseCsv(req.body.reviewer_users),reviewer_roles:parseCsv(req.body.reviewer_roles),enabled:true,created_by:req.session.user.id,created_at:new Date().toISOString()};if(!f.questions.length)return res.status(400).send(page(req,'No Questions','<main class="wrap"><div class="card empty"><h2>Add at least one question.</h2><a class="btn" href="javascript:history.back()">Go Back</a></div></main>'));await createForm(f);res.redirect('/server/'+encodeURIComponent(g.id)+'/forms/'+encodeURIComponent(f.id));});

app.get('/server/:guildId/forms/:formId',auth,async(req,res)=>{const g=manageable(req).find(x=>x.id===req.params.guildId);if(!g)return res.status(403).send('Forbidden');const f=await getForm(req.params.formId);if(!f||f.guild_id!==g.id)return res.status(404).send('Form not found');const subs=await submissions(f.id);const qjson=JSON.stringify(arr(f.questions)).replace(/</g,'\\u003c');const subCards=subs.map(s=>'<div class="card"><div class="row"><div><b>'+esc(s.username)+'</b><div class="muted small">'+esc(s.user_id)+' · '+esc(new Date(s.created_at).toLocaleString())+'</div></div><span class="status '+esc(s.status)+'">'+esc(s.status)+'</span></div><a class="btn" href="/server/'+encodeURIComponent(g.id)+'/forms/'+encodeURIComponent(f.id)+'/submissions/'+encodeURIComponent(s.id)+'">Review</a></div>').join('');const body=`<main class="wrap"><div class="hero"><h1>${esc(f.name)}</h1><p class="muted">Public link:</p><div class="copy">${esc(PUBLIC_URL+'/f/'+f.slug)}</div></div><div class="card"><form method="post" action="/server/${encodeURIComponent(g.id)}/forms/${encodeURIComponent(f.id)}/settings"><div class="grid"><div class="field"><label>Name</label><input class="input" name="name" value="${esc(f.name)}" required></div><div class="field"><label>Type</label><select class="select" name="form_type"><option value="staff">Staff Application</option><option value="appeal">Appeal</option><option value="partnership">Partnership</option><option value="report">Report</option><option value="custom">Custom</option></select></div></div><div class="field"><label>Description</label><textarea class="textarea" name="description">${esc(f.description)}</textarea></div><div class="row"><div><h3 style="margin:0">Questions</h3><div class="muted small">There is no 5-question limit.</div></div><span class="pill" id="questionCount"></span></div><div id="questions"></div><button type="button" class="btn alt" id="addQuestion">＋ Add Question</button><div class="grid" style="margin-top:20px"><div class="field"><label>Reviewer User IDs</label><input class="input" name="reviewer_users" value="${esc(arr(f.reviewer_users).join(', '))}"></div><div class="field"><label>Reviewer Role IDs</label><input class="input" name="reviewer_roles" value="${esc(arr(f.reviewer_roles).join(', '))}"></div></div><div class="field"><label><input type="checkbox" name="enabled" ${f.enabled?'checked':''}> Form enabled</label></div><div class="actions"><button class="btn" type="submit">Save Changes</button><a class="btn alt" href="/server/${encodeURIComponent(g.id)}">Back</a></div></form></div><h2 style="margin-top:30px">Submissions (${subs.length})</h2><div class="list">${subCards||'<div class="card empty">No submissions yet.</div>'}</div></main><script>let qs=${qjson};const escA=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));function syncQuestions(){const box=document.getElementById('questions');if(!box)return;qs=Array.from(box.querySelectorAll('.question')).map(el=>({title:el.querySelector('[name="q_title"]')?.value||'',text:el.querySelector('[name="q_text"]')?.value||'',type:el.querySelector('[name="q_type"]')?.value||'short'}))}function addQ(){syncQuestions();qs.push({title:'',text:'',type:'short'});render()}function render(){document.getElementById('questionCount').textContent=qs.length+(qs.length===1?' question':' questions');const box=document.getElementById('questions');box.innerHTML=qs.map((q,i)=>\`<div class="question"><div class="row"><b>Question \${i+1}</b><button type="button" class="btn danger remove-q">Remove</button></div><div class="qrow"><div class="field"><label>Title</label><input class="input" name="q_title" value="\${escA(q.title)}" required></div><div class="field"><label>Question</label><input class="input" name="q_text" value="\${escA(q.text)}" required></div><div class="field"><label>Answer type</label><select class="select" name="q_type"><option value="short" \${q.type==='short'?'selected':''}>Short text</option><option value="long" \${q.type==='long'?'selected':''}>Long text</option><option value="number" \${q.type==='number'?'selected':''}>Number</option></select></div></div></div>\`).join('');box.querySelectorAll('.remove-q').forEach((b,i)=>b.onclick=()=>{syncQuestions();qs.splice(i,1);render()})}document.getElementById('addQuestion').onclick=addQ;render();</script>`;res.send(page(req,f.name+' · Forms',body));});

app.post('/server/:guildId/forms/:formId/settings',auth,async(req,res)=>{const g=manageable(req).find(x=>x.id===req.params.guildId);if(!g)return res.status(403).send('Forbidden');const f=await getForm(req.params.formId);if(!f||f.guild_id!==g.id)return res.status(404).send('Form not found');let slug=f.slug,requested=slugify(req.body.name);if(requested!==f.slug){const exists=pool?(await pool.query('select id from sparxie_forms where slug=$1 and id<>$2',[requested,f.id])).rows.length:memory.forms.some(x=>x.slug===requested&&x.id!==f.id);if(!exists)slug=requested;}const next={...f,name:String(req.body.name||f.name).trim().slice(0,100),slug,form_type:['staff','appeal','partnership','report','custom'].includes(String(req.body.form_type))?String(req.body.form_type):f.form_type,description:String(req.body.description||'').slice(0,1000),questions:parseQuestions(req.body),reviewer_users:parseCsv(req.body.reviewer_users),reviewer_roles:parseCsv(req.body.reviewer_roles),enabled:req.body.enabled==='on'};await updateForm(f.id,next);res.redirect('/server/'+encodeURIComponent(g.id)+'/forms/'+encodeURIComponent(f.id));});

app.get('/server/:guildId/forms/:formId/submissions/:submissionId',auth,async(req,res)=>{const f=await getForm(req.params.formId);if(!f||f.guild_id!==req.params.guildId)return res.status(404).send('Not found');if(!(await reviewAllowed(req,f)))return res.status(403).send('You are not allowed to review this form.');const s=await getSubmission(req.params.submissionId);if(!s||s.form_id!==f.id)return res.status(404).send('Submission not found');const answers=typeof s.answers==='string'?JSON.parse(s.answers):(s.answers||{});const rows=arr(f.questions).map((q,i)=>{const raw=answers[q.title] ?? answers[encodeURIComponent(q.title)] ?? answers['answer_'+q.title] ?? answers['answer_'+encodeURIComponent(q.title)] ?? answers['answer_'+i];const answer=raw===undefined||raw===null||String(raw).trim()===''?'No answer':String(raw);return '<div class="question"><b>'+esc(q.title)+'</b><div class="muted small">'+esc(q.text)+'</div><p>'+esc(answer)+'</p></div>';}).join('');const body='<main class="wrap"><div class="hero"><h1>Review submission</h1><p class="muted">'+esc(s.username)+' · '+esc(f.name)+'</p></div><div class="card">'+rows+'<form method="post" action="/server/'+encodeURIComponent(f.guild_id)+'/forms/'+encodeURIComponent(f.id)+'/submissions/'+encodeURIComponent(s.id)+'/review"><div class="field"><label>Decision</label><select class="select" name="status"><option value="pending" '+(s.status==='pending'?'selected':'')+'>Pending</option><option value="approved" '+(s.status==='approved'?'selected':'')+'>Approve</option><option value="denied" '+(s.status==='denied'?'selected':'')+'>Deny</option></select></div><div class="field"><label>Review note</label><textarea class="textarea" name="note">'+esc(s.review_note||'')+'</textarea></div><button class="btn" type="submit">Save Review</button></form></div></main>';res.send(page(req,'Review Submission',body));});
...
app.get('/f/:slug',async(req,res)=>{const f=await getSlug(req.params.slug);if(!f)return res.status(404).send(page(req,'Form not found','<main class="wrap"><div class="card empty"><h2>Form not found</h2></div></main>'));if(!req.session.user)return res.redirect('/login?next='+encodeURIComponent(req.originalUrl));const questions=arr(f.questions).map(q=>'<div class="field"><label>'+esc(q.title)+'</label><div class="muted small">'+esc(q.text)+'</div><input class="input" name="answer_'+encodeURIComponent(q.title)+'" '+(q.type==='number'?'type="number"':'type="text"')+' required></div>').join('');res.send(page(req,f.name,'<main class="wrap"><div class="hero"><h1>'+esc(f.name)+'</h1><p class="muted">'+esc(f.description)+'</p></div><form method="post" action="/f/'+encodeURIComponent(f.slug)+'/submit"><div class="card">'+questions+'<button class="btn" type="submit">Submit</button></div></form></main>'));});

app.post('/f/:slug/submit',auth,async(req,res)=>{const f=await getSlug(req.params.slug);if(!f)return res.status(404).send('Form not found');const answers={};for(const [i,q] of arr(f.questions).entries()){const key='answer_'+q.title;const encodedKey='answer_'+encodeURIComponent(q.title);const indexedKey='answer_'+i;const value=req.body[key] ?? req.body[encodedKey] ?? req.body[indexedKey] ?? '';answers[q.title]=String(Array.isArray(value)?value[value.length-1]:value).slice(0,5000);}const s={id:crypto.randomUUID(),form_id:f.id,user_id:req.session.user.id,username:req.session.user.global_name||req.session.user.username||'User',answers,status:'pending',created_at:new Date().toISOString()};await createSubmission(s);res.send(page(req,'Submitted','<main class="wrap"><div class="card empty"><h2>Application submitted</h2><p class="muted">Thanks. Your submission is now pending review.</p><a class="btn" href="/dashboard">Back to dashboard</a></div></main>'));});

app.use((err,req,res,next)=>{console.error(err);res.status(500).send(page(req,'Error','<main class="wrap"><div class="card empty"><h2>Something went wrong</h2><p class="muted">'+esc(err.message||'Unexpected server error')+'</p><a class="btn" href="/dashboard">Back to dashboard</a></div></main>'));});

app.listen(PORT,()=>console.log('Sparxie Forms running on port '+PORT));